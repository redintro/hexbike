...
public class OwnerPersistenceAdapter implements FindOwnerPort {
  ...
  public List<Owner> findAll() {
    ...
    return owners.stream().map(OwnerOutMapper::mapToDomainEntity).collect(Collectors.toList());
  }

  @Override
  public Option<Owner> findById(UUID ownerId) {
    return Option.ofOptional(repository.findById(ownerId).map(OwnerOutMapper::mapToDomainEntity));
  }
}
